...
int main (int argc, char **argv)
{
	...
	/* PAPI Initialization */
     ...
     if ( retval != PAPI_OK) {
        fprintf(stderr,"PAPI_debug failed, %d\n", retval);
	exit(1);
     }
     retval = PAPI_library_init( PAPI_VER_CURRENT );
     fprintf(stderr, "ver_curr: %d\n", PAPI_VER_CURRENT);
	...
	if (strstr(cmpinfo->name,"linux-rapl") || strstr(cmpinfo->name,"rapl")) {
	   rapl_cid=cid;
...
//         usleep(100000);
	   ...
	   fprintf(fff[i],"%.4f %.1f %lld (* Average Power for %s *)\n",
		   total_time,
		   ((double)values[i]/1.0e9)/elapsed_time, values[i],
		   events[i]);
	   fflush(fff[i]);
        }
     }
